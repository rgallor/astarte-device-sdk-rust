    ...
    S::Err: std::fmt::Display,
{
    ...
    async fn load_prop(
        ...
        if property.ownership.is_server() {
            return self
                .store
    ...
    async fn unset_prop(&self, property: &PropertyInfo<'_>) -> Result<(), Self::Err> {
        if property.ownership.is_device() {
...
mod test {
    ...
    async fn test_grpc_store_grpc_client_calls() {
        ...
        let client_operations = async move {
            ...
            let device_prop_info =
                PropertyInfo::new_unchecked((&device_interface).into(), device_path);
            ...
            let server_prop_info =
                PropertyInfo::new_unchecked((&server_interface).into(), server_path);
    ...
    async fn test_grpc_store_device_prop_not_stored() {
        ...
        let prop = base_store
            .load_prop(server_interface_data, 1)
            .await
            .unwrap();
        ...
        let prop = base_store
            .load_prop(device_interface_data, 1)
            .await
            .unwrap();
        assert!(prop.is_none());
