...
impl WriteConnection {
    ...
    pub(crate) fn free_space(transaction: &Transaction, capacity: u64) -> Result<(), SqliteError> {
        ...
        debug!("initial count: {count_stored}");

        if count_stored < capacity {
            return Ok(());
        }

        debug!("store is full, evicting expired properties");
        ...
        debug!("count after expired removal: {count_stored}");
        ...
        debug!("store is still full, evicting the {to_remove} oldest elements");
